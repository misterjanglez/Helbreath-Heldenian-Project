...
- **Which mobs carry the generic Cape (402)** — restricted-mob roster, Plate Mail pattern (§8). **PICKED (2-E, 2026-07-28)**: the placement already present in the live table — 11 stage-1 tables: Demon (20031), Unicorn (20032), Gargoyle (20052), Mountain Giant (20058), Centaurus/Griffin (20085, w=50), Nizie (20089) + all five bosses (20049/20050/20090/20091/20092), w=100 unless noted. `seed_item_tiers.py --place-cape` re-asserts it idempotently.
- **Loot-grade exp thresholds** for the mechanical first pass over the 117 mobs (hand-adjust pass follows regardless). **PICKED (2-E, 2026-07-28)**: by `exp_max` — < 50 vermin, < 2000 standard, < 7000 veteran, ≥ 7000 elite; Apocalypse boss roster (Helclaw/Tiger Worm/Wyvern/Fire Wyvern/Abaddon) explicit grade 5. Owner reviewed and applied unchanged (73/16/15/8/5). NPCs/structures with exp 0 land in vermin — harmless, their tables hold no ordinary gear.
- **Boss stage-1 gear lists** — which §1-scope gear enters each boss's first-drop table, at what weights. **PICKED (2-E, 2026-07-28, owner-approved)**: every boss adds Knight Plate Mail M/W (50/50), Wizard Robe M/W (50/50), Knight Full-Helm M/W (50/50), Knight Plate Leggings M/W (50/50), Tower Shield (100), Composite Bow (100); Helclaw adds Magic Wand MS20 (100, parity with the other four); Abaddon adds Wings Helm M/W (50/50). Gear share rises ~5.5–8% → 11.2–13.6% (sanctioned loosen-later lever per §8 boss-rate note). CSV: `Scripts/output/boss_stage1_gear_proposal.csv`, applied via `--apply-drops`.
...
- **Enchant category derivation** (3-F). **PICKED (3-F, 2026-07-28)**: `enchant_category_for_item_class` (`ModifierIds.h`) maps `tier_item_class` → the seeded category — melee_weapon → weapon (crafted_weapon when custom-made), shield → shield, helm/body_armor/leggings → armor, wand → wand; bow, cape and none carry no category and are not stone-enchantable. Side effect worth recording: the hand-rolled `upgrade_route` chain tested `sub_type == armor` **before** `equip_pos == Back`, so capes fell into the armor route and the hero-cape branch (400/401 → 427/428) was unreachable. Deriving the class restores the original's cape routing (the original switched on the item's `m_cCategory`, 13 = cape) and takes capes out of stone enchanting, which is what §13's cap list already implied.

## Open questions (surface before the affected cycle — not blockers to start)

1. **Enchant per-step success % (needed by Cycle 3-F / seeding step 4).** Spec §10 makes the enchant table data-driven with per-step success %, but the annex specifies only caps, destroy flags, and endurance growth — no success-rate values. Proposed default: extract today's effective per-step success rates from the current handler's formula verbatim into `enchant_steps` (behavior-preserving), leaving tuning to data. Confirm, or supply a launch table. **RESOLVED (2-E, 2026-07-28, proposed default applied)**: extracted verbatim from `check_is_item_upgrade_success` (`ItemManager.cpp:5381`) — base ladder 30/25/20/15/10/10/8/8/5/3% by step, stored as basis points; the Merien-stone route (armor + wand categories) doubles it; crafted quality bonus (`special_effect_value2`) stays code-side. Categories seeded 1 weapon / 2 shield / 3 armor / 4 wand / 5 crafted_weapon (the 3-F code enum). **AMENDED (3-F, 2026-07-28)**: the 2-E seed transposed shield and wand. Verified against the original (`repos/HelbreathServer/HGServer/Game.cpp:52471` shield → `iSomH, TRUE`; `:52795` wand → `iSoxH`), the Merien-stone route is **shield + armor**, not armor + wand — so shield carries the doubled ladder and 15% endurance growth, and wand carries the base ladder and no growth. Corrected in `seed_item_tiers.py` and re-seeded. Also picked here: `endurance_growth_pct` holds the per-category **base**, and the custom-made **+20%** uplift stays code-side, mirroring this question's ruling on the crafted success-% bonus.
2. **Righteous/Kloness fix scope in legacy mode (Cycle 3-F).** §13 says drift fixes apply "both modes unless noted" — so legacy mode also gains the Righteous combat hook and loses the Kloness floor. Reading the contract as written: yes, both modes. Flagging because it changes the retail-faithful mode's live behavior (in the retail-*correct* direction, per #34's evidence). Silence = proceed as written. **RESOLVED (3-F, 2026-07-28, owner-confirmed)**: both modes, as written. The five drift fixes are unconditional except the Ancient-prefix upgrade block, which §13 scopes to tiered mode only.

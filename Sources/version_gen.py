...
BUILD_COUNTER_LAUNCHER = os.path.join(SOURCES_DIR, "build_counter_launcher.txt")
...
def gen_rc_header(sections, build_numbers):
    ...
    for label, ns in [("CLIENT", "client"), ("SERVER", "server"), ("LAUNCHER", "launcher")]:
        fields = sections[ns]
        bn = build_numbers[ns]
        major = int(fields.get("major", 0))
        minor = int(fields.get("minor", 0))
...
def main():
    ...
    parser.add_argument("--target", choices=["client", "server", "launcher", "all"], default="all",
                        help="Which build counter(s) to increment (default: all)")
    args = parser.parse_args()

    try:
        sections = read_version_cfg()
        for required in ("compatibility", "server", "client", "launcher"):
        ...
        increment_launcher = args.increment_version and args.target in ("launcher", "all")
        ...
        build_numbers = {
            ...
            "launcher": read_build_counter(BUILD_COUNTER_LAUNCHER, increment=increment_launcher),
        }
        ...
        header += "\n"
        header += gen_namespace_block("launcher", sections["launcher"], build_numbers["launcher"])
        ...
        print(f"version_gen.py: compat={sections['compatibility'].get('major')}.{sections['compatibility'].get('minor')} "
              ...
              f"build(client={build_numbers['client']}, server={build_numbers['server']}, launcher={build_numbers['launcher']}) -> {OUTPUT_HEADER}")
        return 0

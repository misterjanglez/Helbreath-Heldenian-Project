...
bool DialogBox_TesterMenu::on_click_level_picker(short sX, short sY)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			// The dial reaches the admin ceiling, not the player cap (#116) —
			// the request is just a value; the server grants past m_max_level
			// only to admins-table accounts and clamps everyone else.
			m_selected_level = std::clamp(m_selected_level + deltas[i], 1, hb::shared::limits::AdminMaxLevel);
			audio_manager::get().play_game_sound(sound_type::effect, 14, 5);
			return true;
		}
	}

...
bool NetworkMessageManager::process_message(uint32_t msg_id, char* data, uint32_t msg_size)
{
	...
	if (msg_id == MsgId::Notify)
	{
		...
		case Notify::DerivedStats: NetworkMessageHandlers::HandleDerivedStats(m_game, data); return true;
		...
		case Notify::TesterNpcSearchResult: NetworkMessageHandlers::HandleTesterNpcSearchResult(m_game, data); return true;
#endif // TESTER_ONLY

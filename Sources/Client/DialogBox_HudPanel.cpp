...
{
	if (m_game->m_player->m_super_attack_left <= 0) return;
	...
	bool mastered = (m_game->m_player->m_skill_mastery[combat_system::get().get_weapon_skill_type()] == 100);
	...
	std::string txt = std::format("{}", m_game->m_player->m_super_attack_left);
	if (mastered)

...
void DialogBox_Magic::on_draw()
{
	...
	for (i = 0; i < hb::shared::limits::MaxItems; i++)
	{
		...
		{
			if (player().m_item_list[i]->get_prefix_type() == hb::shared::item::AttributePrefixType::Special)
			{
				v1 = static_cast<double>(result);
				v2 = static_cast<double>(player().m_item_list[i]->get_prefix_value() * m_game->m_prefix_multiplier[10]);
				v3 = v1 + v2;

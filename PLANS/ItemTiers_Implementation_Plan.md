...
- **Unified modifier ID number assignments** — the renumbered-from-scratch table (spec fixes the namespace, not the numbers). Assign in catalog order, document in `ModifierIds.h`.
...
- **Roll-distribution verification harness** (2-C, 3-B) — GM bulk-roll command vs offline seeded-RNG harness.
- **Tier item-class derivation edge cases** — e.g. robes/hauberks map to body armor (spec §1); confirm each `armor_class`/`equip_pos` mapping when writing `tier_item_class`.
